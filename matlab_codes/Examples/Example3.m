%This code is done for Example 3, in which a rational least squares problem is considered
%It Generates Figure 3 related to Example 3
%INPUT:
...
m = 4000;%Sampling nodes
s=logspace(-16,0,m).';
y1=sqrt(s);%Exact function values at sampling nodes


M1=2000;%Nodes and weights
z=logspace(-16,0,M1).';
w=ones(1,M1);
w=w';

f=sqrt(z);%Function values at nodes
...
for n = 1:length(err)
    ...
    [y, H, K] = ratfitZ(z, t, f, n, w);%Rational Arnoldi
    r = ratvalZ(y, H, K, s, r0);

    c = ratfit1(z, f, t, n);%The approach of not employing Arnoldi
    y2 = ratval1(c, s, t);

    c1 = ratfit2(z, f, t, n);%The method introduced in [15]
    y3 = ratval2(c1, s, t);
